...
public class Button implements Serializable{
	...
	public Button(Pad pad, int posX, int posY, int type, int size, int mapping)
	{
		...
		m_downPicture = m_pad.getSVG((m_typeIndex<4)?8:m_typeIndex+5).getPicture();
	...
	public void draw(Canvas canvas, Context context)
	{
		...
		int size = context.getResources().getDimensionPixelSize(R.dimen.size_button_name);
		textPaint.setTextSize(size);
		canvas.drawText(AndroButton.Mapping.values()[m_mappingIndex].name(), m_posX, m_posY+(size/2), textPaint);
	}
	...
	public boolean pressButton(int pointerIndex, float posX, float posY)
	{
		if(m_positionRectangle.contains((int)posX, (int)posY))
		{
			...
			m_pad.sendButtonState(new AndroButton(AndroButton.Mapping.values()[m_mappingIndex], AndroButton.State.PRESSED));
			return true;
		}
	...
	{
		if(pointerIndex == m_pointerIndex && m_positionRectangle.contains((int)posX, (int)posY))
		{
			m_pointerIndex = -1;
			m_pad.sendButtonState(new AndroButton(AndroButton.Mapping.values()[m_mappingIndex], AndroButton.State.RELEASED));
		}
	}
	...
	public boolean moveFinger(int pointerIndex, float posX, float posY) {
		...
		else if(pointerIndex == -1 && m_positionRectangle.contains((int)posX, (int)posY))
		{
			m_pointerIndex = pointerIndex;
			m_pad.sendButtonState(new AndroButton(AndroButton.Mapping.values()[m_mappingIndex], AndroButton.State.PRESSED));
			return true;
	...
	private void updatePosition()
	{
		...
		m_positionRectangle = new Rect(newPosX,newPosY,newPosX+size,newPosY+size);
	}
